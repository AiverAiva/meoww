 ...
 */
async function fetchWNACGMetadata(aid: string): Promise<WNACGMetadata | null> {
...
async function fetchWNACGViewData(
  ...
  try {
    ...
    const highResUrl = normalizeWnacgUrl(
      text.match(/<img[^>]+id="picarea"[^>]+src="([^"]+)"/)?.[1],
    );

    // Page labels - target: <span class="newpagelabel"><b>...</b>/...</span>
    ...
    // Navigation IDs - Use a more flexible regex for the select tag
